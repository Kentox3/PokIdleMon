...
// ── onBattleEnd Override — mit returnToCity-Support ──────────
function onBattleEnd(result) {
  clearInterval(BATTLE_INTERVAL);
  _animRunning = false;
  hideTrainerPortrait();

  // Rival-Kampf in der Stadt → zurück zur Stadtansicht
  if (result === "win" && BATTLE.trainerData && BATTLE.trainerData.returnToCity) {
    setTimeout(function() {
      ...
      setTimeout(function() {
        ...
        var zone = getZone(BATTLE.trainerData.cityZoneId);
        if (zone) renderCityView(zone);
      }, 1500);
    }, 500);
    return;
  }
  ...
  if (result === "win") {
    setTimeout(function() {
      var xp = BATTLE.xpGained || 0, msgs = [], enemyDexId = BATTLE.enemy ? BATTLE.enemy.dexId : null;
