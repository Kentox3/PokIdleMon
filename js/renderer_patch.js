...
// ── updateEnemyHp — KRITISCH: fehlt in renderer.js ───────────
// Ohne diese Funktion → ReferenceError → Kampf friert ein!
...
function renderMoveButtons() {
  ...
  player.moves.forEach(function(mid) {
    ...
    btn.style.borderColor = empty ? "#444" : col;
    ...
    btn.innerHTML =
      ...
      "<span class='move-pp " + (empty?"move-pp-empty":pp<=Math.ceil(ppMax/4)?"move-pp-low":"") + "'>" +
        pp + "/" + ppMax + " AP</span>";
...
function _getGymImgKey(zone) {
  if (!_gymIndexMap) {
    _gymIndexMap = {};
    var n = 0;
    WORLD.forEach(function(z) { if(z.type==="gym"){ n++; _gymIndexMap[z.id]="gym"+n; } });
  }
...
function continueFromCity(){
  ...
  var zone=getZone(STATE.currentZoneId), gym=_getCityGym(zone);
  if(gym&&gym.gymLeader){
    if(STATE.badgeIds.indexOf(gym.gymLeader.badgeId)>=0){
      var a=gym.next; if(a){STATE.currentZoneId=a;STATE.currentStage=1;markZoneVisited(a);
        var az=getZone(a);if(az)renderZoneBg(az);renderStageInfo();renderPlayerSprites();renderWorldTab();
        showToast("⏩ "+gym.name+" übersprungen!");saveGame();startStageLoop();return;}
    }
    if(gym.minBadges&&STATE.badges<gym.minBadges){
      showToast("🔒 "+gym.name+" – "+gym.minBadges+" Orden nötig! (Du: "+STATE.badges+")",4000);
      var a2=gym.next;if(a2){STATE.currentZoneId=a2;STATE.currentStage=1;markZoneVisited(a2);
        var az2=getZone(a2);if(az2)renderZoneBg(az2);renderStageInfo();renderPlayerSprites();renderWorldTab();
...
function advanceStage(){
  ...
  if(STATE.currentStage>zone.stageCount){
    ...
    if(zone.next){
      ...
      if(nz&&nz.type==="gym"&&nz.minBadges&&STATE.badges<nz.minBadges){
        showToast("🔒 "+nz.name+" gesperrt – "+nz.minBadges+" Orden nötig! (Du: "+STATE.badges+")",4000);
        var s=nz.next;if(s){STATE.currentZoneId=s;markZoneVisited(s);var sz=getZone(s);if(sz)renderZoneBg(sz);}
        else{STATE.currentZoneId=zone.next;markZoneVisited(zone.next);}
      }
      else if(nz&&nz.type==="gym"&&nz.gymLeader&&STATE.badgeIds.indexOf(nz.gymLeader.badgeId)>=0){
        var s2=nz.next;if(s2){STATE.currentZoneId=s2;markZoneVisited(s2);var sz2=getZone(s2);if(sz2)renderZoneBg(sz2);}
        else{STATE.currentZoneId=zone.next;markZoneVisited(zone.next);}
      }
      else{
...
function challengeGym(gymZoneId){
  ...
  if(gym.minBadges&&STATE.badges<gym.minBadges){showToast("🔒 "+gym.name+" – "+gym.minBadges+" Orden nötig! (Du: "+STATE.badges+"/8)",4000);return;}
  _waitingForInput=false; _inCity=false;
  STATE.currentZoneId=gymZoneId; STATE.currentStage=1;
  markZoneVisited(gymZoneId); renderZoneBg(gym); renderStageInfo(); renderPlayerSprites(); renderWorldTab();
...
function renderCityView(zone) {
  ...
  if(npc&&!npc._done){
    ...
    npcHtml="<div class='city-npc'><img src='"+npc.sprite+"' class='npc-portrait' onerror='this.style.display=\"none\"'>"+
      ...
      "<div class='npc-trade'><div class='trade-pkmn'><img src='https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/"+npc.give+".png'><span>"+(gPd?gPd.name:"?")+"</span></div>"+
      "<span class='trade-arrow'>⇆</span><div class='trade-pkmn'><img src='https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/"+npc.get+".png'><span>"+(rPd?rPd.name:"?")+"</span></div></div>"+
      "<button class='city-btn trade-btn' onclick='doNPCTrade(\""+zone.id+"\")'>Tauschen</button></div>";
  }
  var rivalHtml="";
  if(zone.cityRival){
    ...
    rivalHtml="<div class='city-rival-block"+(done?" city-rival-done":"")+"'>"+
      "<div class='city-rival-inner'>"+(spr?"<img src='"+spr+"' class='city-rival-portrait' onerror='this.style.display=\"none\"'>":"")+
      "<div class='city-rival-text'>"+(done?"<b>"+zone.cityRival.name+":</b> „Du bist gar nicht schlecht… aber ich werde stärker!"<br><small>✅ Besiegt</small>":"<b>"+zone.cityRival.name+":</b> „Hey! Ich bin "+zone.cityRival.name+"! Ich werde der weltbeste Trainer!" ")+"</div></div>"+
  ...
  if(gym&&gym.gymLeader){
    var gl=gym.gymLeader,badgeEarned=STATE.badgeIds.indexOf(gl.badgeId)>=0,badgesOk=!gym.minBadges||STATE.badges>=gym.minBadges;
    var glSpr=(typeof getGymLeaderSprite==="function")?getGymLeaderSprite(gl.name):"";
    gymHtml="<div class='city-gym-block"+(badgeEarned?" city-gym-beaten":"")+"'>"+
      "<div class='city-gym-inner'>"+(glSpr?"<img src='"+glSpr+"' class='city-gym-portrait' onerror='this.style.display=\"none\"'>":"")+
      "<div class='city-gym-info'><div class='city-gym-name'>⚔️ "+gym.name+"</div><div class='city-gym-leader'>Arenaleiter: <b>"+gl.name+"</b></div>"+
      (gym.minBadges?"<div class='city-gym-req'>"+(badgesOk?"✅":"🔒")+" "+gym.minBadges+" Orden nötig</div>":"")+
      (badgeEarned?"<div class='city-gym-req'>🏅 "+gl.badge+" bereits erhalten</div>":"")+"</div></div>"+
      "<button class='city-gym-btn"+(badgesOk?"":" city-gym-btn-locked")+"' "+(badgesOk?"onclick='challengeGym(\""+gym.id+"\")'":"disabled title='"+(gym.minBadges||"?")+" Orden nötig'")+">"+
      (badgeEarned?"🔄 Nochmal herausfordern":"⚔️ Arena herausfordern")+"</button></div>";
  }
  ...
  container.innerHTML=
    "<div class='city-view'>"+
      "<div class='city-header'><div class='city-title'>🏙️ "+zone.name+"</div>"+
        "<div class='city-subtitle'>"+(rivalPending?"Dein Rival <b>"+zone.cityRival.name+"</b> wartet!":"Du hast die Stadt erreicht – Team geheilt!")+"</div></div>"+
      rivalHtml+gymHtml+
...
function triggerCityRival(zone) {
  ...
  var sm={1:4,4:7,7:1}, rivalDexId=sm[STATE.starter]||4, rivalPd=PKMN[rivalDexId];
  var rivalTrainer={name:zone.cityRival.name,isRival:true,party:[{dexId:rivalDexId,lv:5}],
    reward:zone.cityRival.reward||100,returnToCity:true,cityZoneId:zone.id,rivalKey:rKey};
