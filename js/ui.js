...
var ITEM_DEFS = {
  ...
  // ── VMs (Versteckte Maschinen) ─────────────────────────────
  hm_cut:      { name:"VM01 Zerschneider", desc:"Schneidet Büsche. (Kaskadenmedaille)", img: ITEM_BASE+"hm01.png", isHM:true, hmType:"Normal", lockedBy:"cascade", usageDesc:"Zinnia Arena-Eingang" },
  hm_fly:      { name:"VM02 Fliegen",      desc:"Schnellreise. (Donnermedaille)",       img: ITEM_BASE+"hm02.png", isHM:true, hmType:"Flying",  lockedBy:"thunder", usageDesc:"Schnellreise (noch nicht aktiv)" },
  hm_surf:     { name:"VM03 Surfer",       desc:"Reist über Wasser. (Seelenmedaille)",  img: ITEM_BASE+"hm03.png", isHM:true, hmType:"Water",   lockedBy:"soul",    usageDesc:"Meerrouten" },
  hm_strength: { name:"VM04 Stärke",       desc:"Bewegt Felsbrocken. (Seelenmedaille)",img: ITEM_BASE+"hm04.png", isHM:true, hmType:"Normal",  lockedBy:"soul",    usageDesc:"Siegerstraße (noch nicht aktiv)" },
  hm_flash:    { name:"VM05 Blitz",        desc:"Senkt Genauigkeit. (Steinmedaille)",   img: ITEM_BASE+"hm05.png", isHM:true, hmType:"Normal",  lockedBy:"stone",   usageDesc:"Rotes Felsgebirge (noch nicht aktiv)" },
  // ── Spezial-Items ──────────────────────────────────────────
  fahrrad: {
    name:"Fahrrad",
    desc:"Verdoppelt die Etappengeschwindigkeit auf Routen.",
    img: ITEM_BASE+"bicycle.png",
    isSpecial: true,
    activeDesc:"⚡ 2× Etappentempo aktiv",
  },
  ep_teiler: {
    name:"EP-Teiler",
    desc:"Alle Pokémon in der Party erhalten EP nach jedem Kampf.",
    img: ITEM_BASE+"exp-share.png",
    isSpecial: true,
    activeDesc:"⚡ Ganzes Team erhält EP",
  },
};
...
function renderWorldTab() {
  ...
  if(zone.wildPokemon&&zone.wildPokemon.length>0){
    ...
    zone.wildPokemon.slice().sort(function(a,b){return b.weight-a.weight;}).forEach(function(entry){
      ...
      html+="<div class='encounter-row'><img src='https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/"+entry.dexId+".png' class='enc-sprite' onerror='this.style.opacity=0'><div class='enc-info'><span class='enc-name'>"+(pd?pd.name:"?")+"</span> <span class='enc-lv'>Lv."+entry.minLv+"–"+entry.maxLv+"</span></div><div class='enc-right'><div class='enc-bar-wrap'><div class='enc-bar' style='width:"+Math.min(100,pct*2)+"%'></div></div><span class='enc-pct'>"+pct+"%</span></div></div>";
    });
    html+="</div>";
  }
...
function renderTeamScreen() {
  ...
  container.innerHTML=""; var n=STATE.party.length;
  STATE.party.forEach(function(p,idx){
    var pd=PKMN[p.dexId],name=pd?pd.name:"?";
    var hpPct=Math.max(0,Math.round(p.currentHP/p.maxHP*100)),xpPct=Math.min(100,Math.round(p.xp/p.xpToNext*100));
    var readyEvo=!!p.readyToEvolve,evoPd=readyEvo?PKMN[p.readyToEvolve]:null;
    var card=document.createElement("div"); card.className="team-card"+(p.currentHP<=0?" team-fainted":"")+(readyEvo?" team-evo-ready":"");
    card.innerHTML=
      ...
      "<div class='team-info'>"+
        ...
        (readyEvo?"<button class='team-evo-btn' onclick='triggerEvolution("+idx+")'><img src='https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/"+(p.readyToEvolve||0)+".png' class='team-evo-preview'>✨ Zu "+(evoPd?evoPd.name:"?")+" entwickeln!</button>":"")+
      "</div>"+
      "<div class='team-actions'>"+
        ...
        "<button class='team-act-sm' onclick='sendToBox("+idx+")' title='In Box'>📦</button>"+
      "</div>";
    container.appendChild(card);
  });
  var boxSection=document.getElementById("boxPreview");
  if(boxSection){boxSection.innerHTML=STATE.box.length===0?"<p style='color:#888;text-align:center;padding:12px'>Box ist leer</p>":STATE.box.map(function(p,i){var pd=PKMN[p.dexId];return "<div class='box-mini' onclick='recallFromBox("+i+")'><img src='https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/"+p.dexId+".png'><div>"+(pd?pd.name:"?")+" Lv."+p.level+(p.readyToEvolve?" ✨":"")+"</div></div>";}).join("");}
}
...
function sendToBox(idx){if(!STATE||STATE.party.length<=1){showToast("Mindestens 1 Pokémon!");return;}var p=STATE.party.splice(idx,1)[0];addToBox(p);if(idx===0)renderPlayerSprites();renderTeamScreen();saveGame();showToast((PKMN[p.dexId]?PKMN[p.dexId].name:"?")+" → Box");}
function recallFromBox(idx){if(!STATE||STATE.party.length>=6){showToast("Party voll!");return;}var p=STATE.box.splice(idx,1)[0];STATE.party.push(p);renderTeamScreen();saveGame();showToast((PKMN[p.dexId]?PKMN[p.dexId].name:"?")+" → Party");}

// ══════════════════════════════════════════════════════════════
//  BAG — 4 Kategorien: Spezial / VMs / Schlüssel / Items
// ══════════════════════════════════════════════════════════════
function renderBagScreen() {
  var container=document.getElementById("bagList"); if(!container||!STATE) return;
  container.innerHTML="";

  // ── 1. Spezial-Items (Fahrrad, EP-Teiler …) ───────────────
  var specKeys=Object.keys(ITEM_DEFS).filter(function(k){return ITEM_DEFS[k].isSpecial&&(STATE.items[k]||0)>0;});
  if(specKeys.length>0){
    _bagHeader(container,"⭐ Spezial-Items");
    specKeys.forEach(function(key){
      var def=ITEM_DEFS[key];
      var row=document.createElement("div"); row.className="bag-item bag-special-item";
      row.innerHTML=
        "<div class='bag-icon-wrap'><img src='"+(def.img||"")+"' class='bag-item-sprite' onerror='this.style.display=\"none\"'></div>"+
        "<div class='bag-info'>"+
          "<b>"+def.name+"</b><br>"+
          "<small>"+def.desc+"</small><br>"+
          "<small class='bag-special-active'>"+def.activeDesc+"</small>"+
        "</div>"+
        "<div class='bag-special-badge'>✅ Aktiv</div>";
  ...
  if(hmKeys.length>0){
    _bagHeader(container,"📀 VM-Beutel");
  ...
  if(keyKeys.length>0){
    ...
    keyKeys.forEach(function(key){
      ...
      row.innerHTML=
        ...
        "<span style='font-size:11px;color:#555;padding:4px'>Schlüssel</span>";
      container.appendChild(row);
    });
  }
  ...
  if(useKeys.length>0){
    _bagHeader(container,"💊 Items");
    useKeys.forEach(function(key){
      var count=STATE.items[key]||0,def=ITEM_DEFS[key];
      var row=document.createElement("div"); row.className="bag-item";
      row.innerHTML=
        "<div class='bag-icon-wrap'><img src='"+(def.img||"")+"' class='bag-item-sprite' onerror='this.style.display=\"none\"'></div>"+
        "<div class='bag-info'><b>"+def.name+"</b><br><small>"+def.desc+"</small></div>"+
  ...
  if(specKeys.length===0&&hmKeys.length===0&&keyKeys.length===0&&useKeys.length===0)
    container.innerHTML="<p style='color:#888;text-align:center;padding:20px'>Tasche leer</p>";
}

function _bagHeader(container, label) {
  var h=document.createElement("div"); h.className="bag-section-header"; h.innerHTML=label;
...
function renderMapScreen() {
  ...
  container.innerHTML="";
  var visitedCities=WORLD.filter(function(z){return z.type==="city"&&isZoneVisited(z.id)&&z.id!==STATE.currentZoneId;});
  var travelHeader=document.createElement("div"); travelHeader.className="map-section-title"; travelHeader.textContent="✈ Schnellreise"; container.appendChild(travelHeader);
  if(visitedCities.length>0){
    var grid=document.createElement("div"); grid.className="city-travel-grid";
    visitedCities.forEach(function(zone){var btn=document.createElement("button");btn.className="city-travel-btn";btn.innerHTML="🏙️ "+zone.name;btn.onclick=(function(zid){return function(){fastTravelTo(zid);};})(zone.id);grid.appendChild(btn);});
    container.appendChild(grid);
  }else{var nc=document.createElement("p");nc.className="map-no-travel";nc.textContent="Noch keine weiteren Städte freigeschaltet.";container.appendChild(nc);}
  var curZone=getZone(STATE.currentZoneId);
  if(curZone){var curDiv=document.createElement("div");curDiv.className="map-current-loc";var zIcon={route:"🌿",dungeon:"🕳️",city:"🏙️",gym:"⚔️",sea:"🌊"}[curZone.type]||"📍";curDiv.innerHTML="<b>📍 Du bist hier:</b> "+zIcon+" <b>"+curZone.name+"</b>"+(curZone.stageCount?" — Etappe "+STATE.currentStage+"/"+curZone.stageCount:"");container.appendChild(curDiv);}
  var br=document.getElementById("badgeRow");
  if(br)br.innerHTML=["stone","cascade","thunder","rainbow","soul","marsh","volcano","earth"].map(function(b){return "<span class='badge-icon"+(STATE.badgeIds.indexOf(b)>=0?" badge-earned":"")+"'>🏅</span>";}).join("");
  ...
  WORLD.forEach(function(zone){
    ...
    var row=document.createElement("div"),cls="map-zone map-compact";
...
function buyItem(itemId,cost){
  ...
  // Spezial-Items nur einmal kaufbar
  var def=ITEM_DEFS[itemId];
  if(def&&def.isSpecial&&(STATE.items[itemId]||0)>0){showToast((def.name||itemId)+" besitzt du bereits!");return;}
  ...
  // Fahrrad → Stageloop neu starten mit halber Tickzeit
  if(itemId==="fahrrad"&&typeof startStageLoop==="function"&&!_waitingForInput&&!_inCity) startStageLoop();
  saveGame(); showToast((def?def.name:itemId)+" erhalten!"+(def&&def.activeDesc?" "+def.activeDesc:""));
}
