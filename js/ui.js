...
// ══════════════════════════════════════════════════════════════
//  TEAM-SCREEN
//  FIX: Shiny-Sprites + Shiny-Badge ✨
//  FIX: Box-Wechsel nur in Städten möglich
// ══════════════════════════════════════════════════════════════
function renderTeamScreen() {
  var container=document.getElementById("teamList"); if(!container||!STATE) return;
  container.innerHTML="";
  var n=STATE.party.length;
  var inCity=!!_inCity;   // Box-Buttons aktiv nur in Stadt
  ...
  STATE.party.forEach(function(p,idx){
    ...
    card.className="team-card"
      +(p.currentHP<=0?" team-fainted":"")
      +(readyEvo?" team-evo-ready":"")
      +(isShiny?" team-card-shiny":"");   // ← goldener Rand für Shinys

    card.innerHTML=
      // ── Sprite (Shiny-Version wenn Shiny) ────────────────
      "<img class='team-sprite"+(isShiny?" team-sprite-shiny":"")+"' "+
        "src='"+pkmnSpriteUrl(p.dexId, isShiny)+"' "+
        "onerror='this.src=\""+pkmnSpriteUrl(p.dexId, false)+"\"' "+
        "alt='"+name+"'>"+
      ...
      "<div class='team-info'>"+
        ...
        // ── Typen ─────────────────────────────────────────
        "<div class='team-types'>"+(pd?pd.types.map(function(t){
          return "<span class='type-badge' style='background:"+(TYPE_COLORS[t]||"#aaa")+"'>"+
            (typeof typeName==="function"?typeName(t):t)+"</span>";
        }).join(""):"")+"</div>"+

        // ── HP-Bar ────────────────────────────────────────
        "<div class='team-hprow'>"+
          "<div class='team-hpbar'><div class='team-hpfill' style='width:"+hpPct+"%;background:"+hpColor(p.currentHP,p.maxHP)+"'></div></div>"+
          " <span class='team-hptxt'>"+p.currentHP+"/"+p.maxHP+"</span>"+
        "</div>"+

        // ── XP-Bar ────────────────────────────────────────
        "<div class='team-xprow'>"+
          "<div class='team-xpbar'><div class='team-xpfill' style='width:"+xpPct+"%'></div></div>"+
          " <span class='team-xptxt'>EP "+p.xp+"/"+p.xpToNext+"</span>"+
        "</div>"+

        // ── Moves ─────────────────────────────────────────
        "<div class='team-moves'>"+p.moves.map(function(m){
          var mv=MOVES[m];
          return mv?"<span class='mini-move' style='border-color:"+(TYPE_COLORS[mv.type]||"#888")+"'>"+mv.name+"</span>":"";
        }).join("")+"</div>"+

        // ── Evolutions-Button ──────────────────────────────
        (readyEvo?
          "<button class='team-evo-btn' onclick='triggerEvolution("+idx+")'>"+
            "<img src='"+PKM_SPRITES+(p.readyToEvolve||0)+".png' class='team-evo-preview'>"+
            "✨ Zu "+(evoPd?evoPd.name:"?")+" entwickeln!</button>":"")
      +"</div>"+
      ...
      "<div class='team-actions'>"+
        ...
        // Box-Button: nur in Städten aktiv
        "<button class='team-act-sm"+(inCity?"":" team-act-disabled")+"' "+
          (inCity?"onclick='sendToBox("+idx+")' title='In Box legen'":"title='Nur in Städten möglich' disabled")+
          ">📦</button>"+
      "</div>";
  ...
  if(boxSection){
    if(STATE.box.length===0){
      ...
      boxSection.innerHTML=STATE.box.map(function(p,i){
        var pd=PKMN[p.dexId];
        var isShiny=!!p.shiny;
        var sprUrl=pkmnSpriteUrl(p.dexId, isShiny);
        // Aus-Box-Button: nur in Städten klickbar
        ...
        return "<div class='box-mini"+(isShiny?" box-mini-shiny":"")+"' "+clickAttr+">"+
          ...
          "<div class='box-mini-label'>"+(pd?pd.name:"?")+" Lv."+p.level+
            (p.readyToEvolve?" 🔄":"")+(p.shiny?" ✨":"")+
          "</div>"+
...
(function injectTeamCss(){
  ...
  s.textContent=`
    ...
    .team-card-shiny {
      ...
      background: linear-gradient(135deg, var(--bg-surface) 70%, rgba(245,197,24,.06)) !important;
    }
    .team-sprite-shiny {
      filter: drop-shadow(0 0 5px rgba(245,197,24,.55));
    }
    .team-shiny-badge {
...
// ── Box-Operationen — NUR IN STÄDTEN ──────────────────────────
  ...
  if(specKeys.length>0){
    ...
    specKeys.forEach(function(key){
      ...
      row.innerHTML=
        ...
        "<div class='bag-info'><b>"+def.name+"</b><br><small>"+def.desc+"</small><br><small class='bag-special-active'>"+def.activeDesc+"</small></div>"+
  ...
  if(keyKeys.length>0){
    ...
    keyKeys.forEach(function(key){
      ...
      row.innerHTML=
        ...
        "<span style='font-size:10px;color:#556070;padding:4px'>Schlüssel</span>";
      container.appendChild(row);
    });
  }
