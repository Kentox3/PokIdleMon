...
function _ladeBgBild(zone, callback) {
  var zoneId = typeof zone === "string" ? zone : zone && zone.id;
  if (!zoneId) { callback(null); return; }
  if (_bgCache[zoneId] !== undefined) { callback(_bgCache[zoneId]); return; }
  var datei = typeof zone === "string" ? null : zone && zone.background;
