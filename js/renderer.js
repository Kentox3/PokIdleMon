...
function rendereZoneBg(zone) {
  ...
  _sceneT = 0;
  var fn;
  if      (zone.typ === "see")     fn = () => zeichneMeer(_sceneCtx, _sceneCanvas.width, _sceneCanvas.height, _sceneT);
  else if (zone.typ === "dungeon") fn = () => zeichneHoehle(_sceneCtx, _sceneCanvas.width, _sceneCanvas.height, _sceneT);
  else if (zone.typ === "stadt" || zone.typ === "wachposten") fn = () => zeichneStadt(_sceneCtx, _sceneCanvas.width, _sceneCanvas.height, _sceneT);
  else if (zone.typ === "gym")     fn = () => zeichneArena(_sceneCtx, _sceneCanvas.width, _sceneCanvas.height, _sceneT);
  else                              fn = () => zeichneRoute(_sceneCtx, _sceneCanvas.width, _sceneCanvas.height, _sceneT, zone);
  function loop() { try{fn();}catch(e){} _sceneT++; _sceneAnimId = requestAnimationFrame(loop); }
  loop();
}
...
function holeFxCanvas(){var v=document.getElementById("sceneView");if(!v)return null;if(!_fxCanvas){_fxCanvas=document.createElement("canvas");_fxCanvas.style.cssText="position:absolute;inset:0;width:100%;height:100%;z-index:25;pointer-events:none";v.appendChild(_fxCanvas);_fxCtx=_fxCanvas.getContext("2d");}_fxCanvas.width=v.clientWidth||480;_fxCanvas.height=v.clientHeight||220;return _fxCanvas;}

function fuehreAngriffAnimation(moveTyp, vonSpieler, onTreffer, onFertig) {
  var cv=holeFxCanvas(); if(!cv){if(onTreffer)onTreffer();setTimeout(()=>{if(onFertig)onFertig();},100);return;}
  ...
  function loop(ts){
    if(!start)start=ts; var el=ts-start; c.clearRect(0,0,W,H);
    if(el<320){var p=el/320,e=p<.5?2*p*p:-1+(4-2*p)*p;var cx=srcX+(dstX-srcX)*e,cy=srcY+(dstY-srcY)*e-Math.sin(p*Math.PI)*30;var dg=c.createRadialGradient(cx,cy,0,cx,cy,10);dg.addColorStop(0,fx.glow);dg.addColorStop(.6,fx.col);dg.addColorStop(1,"rgba(0,0,0,0)");c.fillStyle=dg;c.beginPath();c.arc(cx,cy,10,0,Math.PI*2);c.fill();requestAnimationFrame(loop);}
    else{if(!hitDone){hitDone=true;if(onTreffer)onTreffer();}var ip=(el-320)/280;if(ip<=1){c.globalAlpha=1-ip;var dg2=c.createRadialGradient(dstX,dstY,0,dstX,dstY,ip*40);dg2.addColorStop(0,fx.glow);dg2.addColorStop(.5,fx.col);dg2.addColorStop(1,"rgba(0,0,0,0)");c.fillStyle=dg2;c.beginPath();c.arc(dstX,dstY,ip*40,0,Math.PI*2);c.fill();c.globalAlpha=1;requestAnimationFrame(loop);}
    else{c.clearRect(0,0,W,H);if(!fertigDone){fertigDone=true;if(onFertig)onFertig();}}}
  }
...
function werfeBallAnimation(ballTyp, callback) {
  ...
  var scene=document.getElementById("sceneView"); if(!scene){if(callback)callback();return;}
  ...
  function step(ts){
    ...
    img.style.left=(sx+(ex-sx)*e)+"px"; img.style.top=(sy+(ey-sy)*e-H*.2*Math.sin(p*Math.PI))+"px";
    img.style.transform=`rotate(${p*540}deg)`;
    if(p<1){requestAnimationFrame(step);return;}
    var w=0,wd=1,wi=setInterval(()=>{w++;wd=-wd;img.style.transform=`rotate(${wd*12}deg)`;if(w>=5){clearInterval(wi);img.style.transform="";setTimeout(()=>{if(img.parentNode)img.parentNode.removeChild(img);if(callback)callback();},150);}},140);
  }
...
function fuegeKampfLogHinzu(zeilen) {
  ...
  zeilen.forEach(z=>{if(!z)return;var p=document.createElement("p");p.textContent=z;log.appendChild(p);while(log.children.length>40)log.removeChild(log.firstChild);});
