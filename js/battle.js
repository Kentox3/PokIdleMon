// ═══════════════════════════════════════════════════════════════
//  battle.js — Gen-1-Kampfsystem (Overhaul)
// ═══════════════════════════════════════════════════════════════
...
//  GESCHLECHT
// ══════════════════════════════════════════════════════════════
// Weiblich-Wahrscheinlichkeit (0–1), null = geschlechtslos
var GENDER_RATIO = {
  // Starter-Linien: 12,5% weiblich
...
function generateGender(dexId) {
  if(GENDER_RATIO.hasOwnProperty(dexId)){
    var r=GENDER_RATIO[dexId];
    if(r===null) return null;
    return Math.random()<r?"F":"M";
  }
...
function autoPickMove(attacker, defender) {
  ...
  usableMoves.forEach(function(mid){
    ...
    if(move.pwr>0){
      ...
      else if(et2==="heal50"||et2==="fullheal"||et2==="regeneration"){score=hpPct<0.35?100:hpPct<0.6?40:0;}
...
function applyStatusDamage(pkmn) {
  ...
  if(pkmn.status==="burn"||pkmn.status==="poison"){var dmg=Math.max(1,Math.floor(pkmn.maxHP/16));pkmn.currentHP=Math.max(0,pkmn.currentHP-dmg);return name+" leidet! (-"+dmg+" HP)";}
  if(pkmn.status==="sleep"){pkmn.statusTurns=(pkmn.statusTurns||1)-1;if(pkmn.statusTurns<=0){pkmn.status=null;return name+" ist aufgewacht!";}return name+" schläft...";}
  return null;
}
...
function startBattle(type, data) {
  ...
  if(type==="wild"){
    enemyPkmn=data;
    // Gender setzen falls fehlt
    if(enemyPkmn.gender===undefined) enemyPkmn.gender=generateGender(enemyPkmn.dexId);
  } else {
    queue=data.party.map(function(e){var p=createPkmnInstance(e.dexId,e.lv);return p;});
  ...
  BATTLE={
    ...
    playerMoveQueued:null, // Manuell gewählte Attacke
  };
  return BATTLE;
}
...
// ══════════════════════════════════════════════════════════════
//  SPIELER-ATTACKE
//  FIX: Keine auto-Umleitung wenn Spieler explizit einen Move wählt.
//       Wenn PP=0 → Kräftemessen. Nicht autoPickMove für Spieler-Input.
// ══════════════════════════════════════════════════════════════
function doPlayerAttack(moveId) {
  ...
  if(player.status==="freeze"){
    if(Math.random()<0.2){player.status=null;log.push(name+" ist aufgetaut!");}
    else{log.push(name+" ist eingefroren!");return log;}
  }

  // ── FIX: Kein autoPickMove für Spieler-Moves ─────────────────
  // Wenn alle PP leer → Kräftemessen
  if(!hasPP(player)){
    moveId="struggle";
  } else if(moveId!=="struggle"&&!moveHasPP(player,moveId)){
    // Gewählter Move hat keine PP mehr — sollte nicht passieren wenn Buttons korrekt disabled sind
    // Sicherheits-Fallback: Kräftemessen statt falscher Attacke
    moveId="struggle";
    log.push("Keine AP mehr! "+name+" setzt Kräftemessen ein!");
  }
  ...
  if(move.pwr>0){
    ...
    if(moveId==="struggle"){var recoil=Math.max(1,Math.floor(player.maxHP/4));player.currentHP=Math.max(0,player.currentHP-recoil);log.push(name+" erleidet Rückstoßschaden! (-"+recoil+" HP)");}
  }
...
function doEnemyAttack() {
  ...
  if(enemy.status==="sleep"){enemy.statusTurns=(enemy.statusTurns||1)-1;if(enemy.statusTurns<=0){enemy.status=null;log.push(ename+" ist aufgewacht!");}else{log.push(ename+" schläft...");return log;}}
  if(enemy.status==="paralysis"&&Math.random()<0.25){log.push(ename+" ist voll gelähmt!");return log;}
  if(enemy.status==="freeze"){if(Math.random()<0.2){enemy.status=null;log.push(ename+" ist aufgetaut!");}else{log.push(ename+" ist eingefroren!");return log;}}
...
function checkBattleEnd() {
  ...
  if(BATTLE.enemy.currentHP<=0){
    ...
    else{BATTLE.moneyGained=Math.max(1,Math.floor(BATTLE.enemy.level/2));}
  ...
  if(justFainted){
    ...
    if(!nextAlive){BATTLE.over=true;BATTLE.result="lose";log.push("Alle Pokémon sind K.O.!");log.push("Du verlierst das Bewusstsein...");return{log:log,over:true,result:"lose"};}
...
// ══════════════════════════════════════════════════════════════
//  FANGEN — FIX: Korrekt dekrementieren + Feedback
// ══════════════════════════════════════════════════════════════
...
function useBattleItem(itemKey) {
  ...
  switch(itemKey){
    ...
    case "revive":{
      ...
      appendBattleLog((PKMN[fainted.dexId]?PKMN[fainted.dexId].name:"?")+" wurde belebt!"); break;
    }
